...
9. Use webSearch whenever a user asks about real-time web information, tourist attractions, sights to see, places to visit, weather forecasts, or travel directions for a destination (such as Goa, Mumbai, Manali). Always call webSearch instead of declining.
"""
...
async def process_chat(message: str, history: List[Dict[str, Any]], user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    messages = [
        ...
        *[
            ...
            for m in history[-6:]
            if (m.get("content") or m.get("text")) and "virtual booking assistant" not in (m.get("content") or m.get("text"))
        ],
